...
class Run(mjx_env.MjxEnv):
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        action = state.info["motor_failure_mask"] * action
        ...
        state = jax.lax.cond(
            ...
            lambda: self._maybe_switch_off_actuators(
                state, self._config.num_failures
            ),
            lambda: state,
        )
    ...
    def _maybe_switch_off_actuators(
        self, state: mjx_env.State, num_switch_offs: int
    ) -> mjx_env.State:
        rng, permute_rng = jax.random.split(state.info["rng"])

        zero_indices = jax.random.choice(
            key=permute_rng,
            a=self.mjx_model.nu,
            shape=(num_switch_offs,),
            replace=False,
        )

        motor_failure_mask = jp.ones((self.mjx_model.nu,), dtype=jp.float32)
        motor_failure_mask = motor_failure_mask.at[zero_indices].set(0.0)

        new_info = state.info.copy()
        new_info["motor_failure_mask"] = motor_failure_mask
        new_info["rng"] = rng
