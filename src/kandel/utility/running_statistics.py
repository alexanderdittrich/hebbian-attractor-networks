...
def update(
    ...
    *,
    ...
    assert jax.tree_util.tree_structure(batch) == jax.tree_util.tree_structure(
        state.mean
    )
    batch_shape = jax.tree_util.tree_leaves(batch)[0].shape
    # We assume the batch dimensions always go first.
    batch_dims = batch_shape[
        : len(batch_shape) - jax.tree_util.tree_leaves(state.mean)[0].ndim
    ]
    ...
    def compute_std(
        summed_variance: jnp.ndarray, std: jnp.ndarray
    ) -> jnp.ndarray:
...
def normalize(
    ...
    return jax.tree_util.tree_map(
        normalize_leaf, batch, mean_std.mean, mean_std.std
    )


def denormalize(batch: ArrayLike, mean_std: NestedMeanStd) -> ArrayLike:
    ...
    return jax.tree_util.tree_map(
        denormalize_leaf, batch, mean_std.mean, mean_std.std
    )
