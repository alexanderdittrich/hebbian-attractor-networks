...
def get_flatten_fcn(pytree):
    """Return functions to flatten and reconstruct a PyTree solution."""

    def flatten_fcn(pytree):
        flat, _ = flatten_util.ravel_pytree(pytree)
        return flat

    _, unflatten_fcn = flatten_util.ravel_pytree(pytree)

    return flatten_fcn, unflatten_fcn
